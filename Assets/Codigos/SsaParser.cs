...
namespace SubtitlesParser.Classes.Parsers
{
    ...
    public class SsaParser : ISubtitlesParser
    {
        ...
        public List<SubtitleItem> ParseStream(string ssaStream)
        {
            ...
            StreamReader reader = new StreamReader(ssaStream, System.Text.Encoding.GetEncoding("iso-8859-1"));
                    ...
                    if (startIndexColumn > 0 && endIndexColumn > 0 && textIndexColumn > 0)
                    {
                        ...
                        while (line != null)
                        {
                            if (!string.IsNullOrEmpty(line))
                            {
                                ...
                                string textLine = "";
                                for (int i = textIndexColumn; i < columns.Length; i++)
                                {
                                    textLine += columns[i] + ",";
                                }
                                textLine = textLine.Substring(0, textLine.Length - 1).Replace("\\N", "\n").Replace("{\\i1}", "").Replace("{\\i0}", "");
                                //textLine = columns[textIndexColumn].Replace("\\N", "\n").Replace("{\\i1}", "").Replace("{\\i0}", ""); // string.Join(",", columns.Skip(textIndexColumn));
                                ...
                                if (start > 0 && end > 0 && !string.IsNullOrEmpty(textLine))
                                {
                                    var item = new SubtitleItem()
                                    {
                                        StartTime = start,
                                        EndTime = end,
                                        Lines = textLine //new List<string>() { textLine }
                                    };
                                    items.Add(item);
                                }
                            }
        ...
        {
            int result;
            if (int.TryParse((s + "0").Replace(".", "").Replace(":", ""), out result))
            {
                return result;
            }
