...
namespace SubtitlesParser.Classes.Parsers
{
    ...
    public class SrtParser: ISubtitlesParser
    {
        ...
        public List<SubtitleItem> ParseStream(string srtStream)
        {
            ...
            StreamReader reader = new StreamReader(srtStream, System.Text.Encoding.GetEncoding("iso-8859-1"));
            ...
            if (srtSubParts.Any())
            {
                foreach (var srtSubPart in srtSubParts)
                {
                    ...
                    if ((item.StartTime != 0 || item.EndTime != 0) && item.Lines != "")
                    {
                        // parsing succeeded
                        items.Add(item);
                    }
        ...
        private static int ParseSrtTimecode(string s)
        {
            ...
            if (match.Success)
            {
                s = match.Value;
                int result;
                if (int.TryParse(s.Replace(",", "").Replace(":", ""), out result))
                {
                    return result;
                }
            }
